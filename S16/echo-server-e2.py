...
class TestHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if "GET / " in self.requestline and not "msg=" in self.requestline:
            contents = open("html/form-e2.html", "r").read()
        elif "GET /echo" in self.requestline and not "msg=" in self.requestline:
            contents = open("html/form-e2.html", "r").read()
        elif "msg=" in self.requestline and "&chk=on" in self.requestline:
            i = self.requestline.find("=")
            name = self.requestline[i + 1:]
            name = name.replace("&chk=on HTTP/1.1", "").upper()
            contents = read_html_file("echo.html").render(context={"todisplay": name})
