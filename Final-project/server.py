    ...
    def do_GET(self):
        ...
        if parsed_path == "/" or parsed_path == "/index" or parsed_path == "/info/index":
            ...
            if ERROR:
                ...
                contents = read_html_file("listSpecies.html").render(context={"length": length, "limit": limit, "species": species})
            content_type = 'text/html'

        elif parsed_path == "/Karyotype":
            common_name = parameters["species"][0]
            species_list = []
            ...
            ENDPOINT = "/overlap/region/human/" + chromo + ":" + start + "-" + end + "?feature=gene;feature=transcript;feature=cds;feature=exon;"
            s = ServerFormat(ENDPOINT)
            response = s.get_response()
